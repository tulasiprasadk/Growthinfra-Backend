...
export class SearchController {
  ...
  async search(@Body() body: { query?: string; context?: any }) {
    ...
    const extractKeywords = (value: string, max = 6): string[] => {
      const stop = new Set([
    ...
    const primaryTopicSource = [description, imagePrompt, normalizedQuery].find((v) => String(v || '').trim().length > 0) || normalizedQuery;
    const detectedKeywords = extractKeywords(primaryTopicSource, 8);
    ...
    const sanitizeInsightText = (text: string) => {
      ...
      // Avoid parroting the exact raw query sentence as output.
      const normalizedClean = cleaned.toLowerCase().replace(/\s+/g, ' ').trim();
      const normalizedTopic = normalizedQuery.toLowerCase().replace(/\s+/g, ' ').trim();
      if (normalizedClean === normalizedTopic || normalizedClean.startsWith(`${normalizedTopic}.`)) {
        const brand = brandName || 'our brand';
        const offer = description || 'clear value and measurable results';
        const cta = website ? `Learn more at ${website}.` : 'Contact us to get started.';
        cleaned = `${brand} helps businesses improve visibility and steady growth with ${offer}. ${cta}`;
      }
    ...
    const buildFallbackInsights = (topic: string, count: number) => {
      ...
      if (topicLower.includes('vegetable') || topicLower.includes('fresh') || topicLower.includes('food') || topicLower.includes('deliver')) {
        ...
        value = `We help businesses become more discoverable and grow consistently through ${offerings}. ${taglineText}`;
    ...
    if (imagePrompt) {
      ...
      const c = combinedContext.toLowerCase();
      if (c.includes('seo') || c.includes('visibility') || c.includes('discoverable') || c.includes('marketing')) {
        imageQueryParts.push('local business marketing analytics dashboard');
      } else if (c.includes('clinic') || c.includes('health') || c.includes('medical')) {
        imageQueryParts.push('modern clinic healthcare consultation');
      } else if (c.includes('food') || c.includes('restaurant') || c.includes('grocery') || c.includes('delivery')) {
        imageQueryParts.push('fresh produce grocery delivery');
      } else if (c.includes('real estate') || c.includes('property')) {
        imageQueryParts.push('real estate property showcase');
      } else if (description) {
    ...
    try {
      if (this.aiService.isLlmEnabled) {
        const r = await this.aiService.generateReels(normalizedQuery, 1, requestedVideoDuration);
    ...
    const generateHashtags = (): string => {
      ...
      // 2) Topic keywords
      const topicWords = extractKeywords(`${description} ${audience} ${normalizedQuery}`, 6).filter(w => !stopwords.has(w));
